...
sudo openssl req -newkey rsa:2048 -nodes -keyout /etc/pki/tls/private/royalt.key -x509 -days 365 -out /etc/pki/tls/certs/royalt.crt \
-subj "/C=NG/ST=Abuja/L=Kubwa/O=mosesitoya.link/OU=devops/CN=$(curl -s http://169.254.169.254/latest/meta-data/local-hostname)"


sudo sed -i 's/localhost.crt/royalt.crt/g'  /etc/httpd/conf.d/ssl.conf

sudo sed -i 's/localhost.key/royalt.key/g'  /etc/httpd/conf.d/ssl.conf
